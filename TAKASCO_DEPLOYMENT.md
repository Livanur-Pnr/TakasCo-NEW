...
- [ ] Telefon numarası (SMS) doğrulaması entegre et: şu an yalnızca e-posta doğrulaması var. Mimari zaten kurulu (e-posta doğrulama kodundaki 6 haneli kod + hash + süre mantığı aynen kullanılabilir, sadece bildirim kanalı SMS API'sine değişir). Önerilen sağlayıcı: NetGSM (BTK lisanslı, Türkiye'ye özel en ucuz seçenek, ~0.20-0.48 TL/SMS). Şimdilik ertelendi çünkü gerçek bir alan adı/kayıt henüz yok, geliştirme aşamasında SMS ücreti ödemek istemiyoruz — canlıya geçerken NetGSM hesabı açılıp (şirket/vergi no + gönderen "başlık" onayı gerekebilir) entegre edilecek.
