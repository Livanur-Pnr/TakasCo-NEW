...
## Düğme animasyonları
Kimlik doğrulama ekranları `@/components/auth/auth-shell` (`AuthShell`, `AuthHeading`, `AuthItem`), `auth-field` (`AuthField`) ve `feature-chips` ile kurulur; yeni bir auth ekranı bu iskeleti kullanmalıdır. `AppTextInput`'a `bare` verilirse kendi odak stilini uygulamaz (sarmalayıcı yönetiyorsa). Marka simgesi: `@/components/brand/takasco-mark` (`TakascoMark`) — TakasCo sembolünün SVG hâli, `variant` (light/dark/mono/reverse) ve `animate` (entrance/success/none) alır; yeni bir yerde logo gerekirse statik PNG yerine bu kullanılmalı. "TakasCo" yazısı logo olarak gösterilecekse düz metin yerine `@/components/brand/takasco-wordmark` (`TakascoWordmark`, `size`, `variant`: auto/dark/light/brand) kullanılmalı; bu bileşendeki sürekli ışık bandı, genel "parlama yok" kuralına kullanıcı isteğiyle yapılmış bir istisnadır. Backend'de e-posta bildirimlerinin (Laravel `MailMessage`) başlığındaki logo `backend/public/images/mail-logo.png` (aynı geometrinin statik render'ı) — `backend/resources/views/vendor/mail/html/header.blade.php` bunu `config('app.name')` eşleştiğinde gösterir. Tema: `useTheme()` artık `hooks/use-app-theme.tsx`'teki (Açık/Koyu/Sistem) çözümlenen paleti döndürür; yeni renk eklerken `constants/theme.ts`'teki `Colors.light` VE `Colors.dark`'a birlikte eklenmeli. `ThemedText`/`ThemedView` zaten tema-duyarlı; doğrudan `'#fff'`/`'#000'` gibi sabit renkler yalnızca (marka rengi düğme üstü beyaz metin gibi) temadan bağımsız olması gereken yerlerde kullanılmalı. Yeni ortak bileşenler: metin girişi için `@/components/ui/text-input` (RN `TextInput` yerine), modal için `@/components/ui/animated-modal` (RN `Modal` yerine), formlarda `@/components/ui/form` (`FormError`, `ActionButton`, `PasswordInput`), süre/eğri/mesafe için `@/constants/motion`. Yalnızca `transform`/`opacity` animasyonu; zıplama, dönme, parlama yok.
