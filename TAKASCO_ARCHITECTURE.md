...
- WebSocket'e geçiş (yapılmadı): `laravel/reverb` paketi kurulmalı (`composer require laravel/reverb`, `php artisan reverb:install`), `MessageSent` olayı `ShouldBroadcast` ile `private-conversation.{id}` kanalına yayınlanmalı, kanal yetkisi `Conversation::hasParticipant` ile verilmeli ve istemciye Echo/pusher-js eklenmelidir. Ayrı bir sunucu süreci (`php artisan reverb:start`) gerekir; polling yedek olarak kalabilir.
