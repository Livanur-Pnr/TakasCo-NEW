...
## Mesajlaşma cilası
`MessagesRead` olayı (`conversation.{id}`), karşı taraf okunmamış mesajı olan bir konuşmayı açınca yayınlanır (okunmamış yoksa yayın yok) ve gönderende ✓ → ✓✓ olur. `GET /conversations/{id}/messages` artık `?before={id}` (50'şer eski mesaj) ve `has_more` destekler. "Yazıyor…" sunucuya uğramayan istemci-istemci `whisper` olayıdır (Reverb `accept_client_events_from=members`). Bildirim sınıfları `broadcast` kanalını da kullanır (`onConnection('sync')`, kuyruk işçisi gerekmez); istemci Echo'nun bildirim olayını dinleyip zili anında yeniler.

## İlan yaşam döngüsü ve fotoğraflar
`products.status=5` **rezerve**: herkese açık listelerde görünür (`Product::published()` 1,2,5 içerir) ama yeni takas teklifi almaz (409) ve eşleşme/öneri dışıdır; `POST /products/{id}/reserve` açıp kapatır. `expires_at`: yeni ilanlar 60 gün yayında kalır (`Product::LISTING_DAYS`), süresi dolan ilan `published()` dışında kalır, detayı yalnızca sahibi/admin görür, `is_expired` alanı sahibe gösterilir; `POST /products/{id}/renew` süreyi yeniden başlatır. Mevcut ilanlar (`expires_at` boş) süresizdir. Süre dolmadan önce bildirim/e-posta yoktur (zamanlanmış görev kurulmadı). `PUT /products/{id}/images/order` (`ids` = tüm fotoğrafların yeni sırası, ilki kapak); `Product::images()` `sort_order`'a göre sıralıdır. İstemci web'de yüklemeden önce görseli tarayıcıda küçültür (1600 px, JPEG %82) ve yükleme ilerlemesini XHR ile gösterir; ilan formu metin taslağını cihazda saklar.

## Profil ve yönetim
`users.bio` (≤ 300 karakter, herkese açık). `GET /users/{id}/products?status=traded` takaslanmış ilanları döner. Yönetim: `GET /admin/reviews` (`rating`, `q` filtreleri), `DELETE /admin/reviews/{id}`, `GET /admin/actions`; ilan kaldırma/geri yükleme, askıya alma/kaldırma, şikayet sonuçlandırma ve değerlendirme silme `admin_actions` tablosuna kaydedilir (kayıt hatası işlemi bozmaz, başarısız işlem kaydedilmez).

## Google ile giriş
`POST /api/auth/google` (`throttle:10,1`, `{credential}`): istemci (web, Google Identity Services) Google'dan aldığı ID token'ı gönderir. `GoogleTokenVerifier` RS256 imzayı Google'ın JWKS anahtarlarıyla (`www.googleapis.com/oauth2/v3/certs`, 1 saat önbellek) yerelde doğrular; `iss`, `aud` (= `GOOGLE_CLIENT_ID`), `exp`, `sub` ve `email_verified` denetlenir; istemci kimliği tanımlı değilse giriş kapalıdır (401). Hesap eşleme: `users.google_id` (gizli alan) → yoksa e-posta ile mevcut hesap → yoksa yeni hesap (parolasız, e-posta doğrulanmış, `is_new=true` ile onboarding'e gider). **Ön-ele geçirme koruması:** e-postası hiç doğrulanmamış mevcut hesap Google ile devralınırsa eski parolası geçersiz kılınır ve oturumları kapatılır. Askıdaki hesap 403. İstemci sırrı kullanılmaz. Native uygulamada Google/Apple girişi yoktur (ayrı istemci kimlikleri ve özel build gerekir).
